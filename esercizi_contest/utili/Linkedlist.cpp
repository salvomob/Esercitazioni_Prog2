...
usiamo la memoria come un enorme array. Gli elementi della lista sono collegati tramite puntatori (in questo caso il next, poiche singolarmente linkata)
...
public:
	...
	LinkedList<T>& ins(T key){
		...
		// ci sono tre casi: ins in testa, ins in mezzo alla lista , ins in coda
		// caso 1 e 2 : inserimento in mezzo alla lista e in coda
	...
	LinkedList<T>&  canc(T key){
		...
		Nodo<T> *y = x->getNext();
		// andiamo a cercare il corretto valore di p
		/*Nodo<T> *tmp = head;
		p=NULL;
		while(tmp!=x){
			p = tmp;
...
int main(){
	LinkedList<int> l ;
	l.ins(11.6).ins(7).ins(15).ins(4).ins(10);
	cout << l << endl;
