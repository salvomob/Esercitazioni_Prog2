...
public:
    ...
    T getLevelSum(int l){
    	...
    	Nodo<T>* tmp = _min(root);
    	while(tmp){
    		if(_prof(tmp->getKey(),root)==l) sum += (int)tmp->getKey();
    		tmp = _successore(tmp);
    	}
    	return sum;
    }
  ...
  int getMaxPath(){
  	Nodo<T>* tmp = _min(root);
  	int maxi=0;int path = 0;
  	while(tmp){
  		path = getPath(tmp->getKey());
  		if(maxi < path) maxi = path;
  		tmp = _successore(tmp);
  	}
  	return maxi;
  }
  ...
  T getMPK(){
  	...
  	while(tmp){
  		path = getPath(tmp->getKey());
  		if (path == getMaxPath()) return tmp->getKey();
  		else tmp = _successore(tmp);
  	}
  	return 0;
  }
  
  void printMaxPath(ostream& out,T key){
  	Nodo<T>* tmp = root;
  	Nodo<T>* x = _search(root,key);
  	int a = getMaxPath();
  	for(int i = 0; i < a; i++ ){
  	...
  	int distanceBetweenTwo(T key1,T key2){
   		Nodo<T>* x = root;
   		int ret = 0;
   		if(key1 == x->getKey()) return (getPath(key2)-1);
   		if(key2 == x->getKey()) return (getPath(key1)-1);
   		if(x->getKey() >= key1 && x->getKey()<= key2){
   			int a = getPath(key1)-1;
   			int b = getPath(key2)-1;
   			ret = a+b;
   			return ret;
   		}
   		if(x->getKey() <= key1 && x->getKey()>= key2){
