   ...
   void scambia(T *v, int i, int j){
	...
	void stampaUscenti(T key,ostream& out){
		...
		for(int j = 0 ; j < cont1 ; j++){
			if(cont > j && cont != j+1) out << b[j] << " ";
			if(j==cont|| j == (cont-1))out << b[j]; 
			}
		out << ")";
	...
	int topologicalSort(ostream& out){
		if(tps()!=0){
    ...
    int i  =_searchNode(key);
		...
		for(int j = 0 ; j < n ; j++){
			if(adj[i][j] == 1  ){
				cont1++;
				if(cont1<cont) out << *k[j] << " ";
				else if(cont1==cont) out << *k[j]; 
			}
		}
		out << ")" <<endl;
    }
